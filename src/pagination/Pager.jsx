      ...
      <ul onClick={this.onPagerClick.bind(this)} className="el-pager">
...

        {
          pagers.map((pager, idx)=>{
            return (
              <li key={idx} className={this.classNames('number', {active: currentPage === pager})}>{pager}</li>
            )
          })
        }
        ...
        {pageCount > 1 && <li className={ this.classNames('number', {active: currentPage === pageCount})}>{pageCount}</li>}
      </ul>
    );
  }
}
