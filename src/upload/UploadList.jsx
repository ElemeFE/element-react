...
export default class UploadList extends Component {
  ...
  render() {
    ...
    return (
      <Transition component="ul" className={this.classNames({
        'el-upload-list': true,
      ...
      >
        {
          fileList.map(file =>
            ...
            >
              ...
              <a className="el-upload-list__item-name" onClick={() => onPreview(file)}>
                <i className="el-icon-document"></i>{file.name}
              </a>
              <View className="el-upload-list__item-status-label" show={isFinished(file.status)} component="label">
                <i className={this.classNames({
                  'el-icon-circle-check': listType === 'text',
                  'el-icon-check': ['picture-card', 'picture'].includes(listType)
                })}></i>
                <i className="el-icon-close" onClick={() => onRemove(file)}></i>
              </View>
              <View className="el-upload-list__item-actions" show={listType === 'picture-card' && isFinished(file.status)}>
                <span
